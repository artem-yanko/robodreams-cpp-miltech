...
static double estimateTravelTime(double distance, double startSpeed, double attackSpeed, double acceleration) {
    if (distance <= 0.0 || attackSpeed <= 0.0) {
        return 0.0;
    }

    if (acceleration <= 0.0) {
        return startSpeed > 0.0 ? distance / startSpeed : distance / attackSpeed;
    }
    ...
    if (distance <= distanceToFullSpeed) {
        return (-startSpeed + std::sqrt(startSpeed * startSpeed + 2.0 * acceleration * distance)) / acceleration;
    }
...
static Coord normalizedOrZero(const Coord& value) {
    ...
    return valueLength > 0.0 ? value / valueLength : Coord{};
}
...
static bool estimateTimeToCompleteMove(
    double& totalTime,
    double& currentSpeed,
    double& currentDir,
    ...
    double goalDir = std::atan2(deltaToGoal.y, deltaToGoal.x);
    double deltaAngle = std::fabs(calculateAngleDifference(currentDir, goalDir));
...
static bool estimateTimeToTargetPath(
    double& totalTime,
    ...
    if (currentTargetIndex != -1 && currentTargetIndex != targetIndex) {
        ...
        if (isMoving || isAccelerating || isDecelerating) {
            if (acceleration > 0.0 && currentSpeed > 0.0) {
                ...
                localPos += {std::cos(localDir) * stopDistance, std::sin(localDir) * stopDistance};
            }
        }
...
Target TargetAnalyzer::analyzeTarget(const MissionState& state, std::size_t targetIndex) const {
    Target result{};
    if (targetIndex >= state.targets.size()) {
        return result;
    }

    const dlink::TargetPos& target = state.targets[targetIndex];
    result.position = Coord{target.x, target.y};
    if (targetIndex < state.targetTracks.size() && state.targetTracks[targetIndex].hasVelocity) {
        result.velocity = state.targetTracks[targetIndex].velocity;
    }
    return result;
}

Coord TargetAnalyzer::predictTargetPosition(const Target& target, double flightTime) const {
    return target.position + target.velocity * flightTime;
}

void TargetAnalyzer::calculateDropPoint(
    Coord& dropPoint,
    Coord& maneuverPoint,
    bool& needManeuver,
...
bool TargetAnalyzer::evaluateTarget(
    ...
    Target target = analyzeTarget(state, targetIndex);
    Coord dropPoint{};
    Coord maneuverPoint{};
    ...
    double remainingAccelerationDistance = distanceToFullSpeed(droneMotion.currentSpeed, config.attackSpeed, acceleration);
    ...
    for (int iteration = 0; iteration < TARGET_PREDICTION_ITERATIONS; ++iteration) {
        ...
        if (length(releaseVector) > 0.0) {
            releaseHeading = std::atan2(releaseVector.y, releaseVector.x);
        }

        if (returningFromManuver && static_cast<int>(targetIndex) == droneMotion.currentTargetIndex) {
            needManeuver = false;
        }
        ...
        if (!estimateTimeToTargetPath(
                totalTime,
                dronePosition,
                droneMotion.currentSpeed,
                droneMotion.currentDir,
                isTurning,
                isMoving,
                isDecelerating,
                isAccelerating,
                droneMotion.currentTargetIndex,
                static_cast<int>(targetIndex),
                droneMotion.turnTargetDir,
                droneMotion.turnRemainingTime,
                needManeuver,
                maneuverPoint,
                dropPoint,
                config.attackSpeed,
    ...
    if (!alreadyAtAttackSpeed && distanceToDropPoint <= requiredAccelerationDistance + feasibilityMargin) {
    ...
    best.targetIndex = static_cast<int>(targetIndex);
...
bool TargetAnalyzer::selectBestTarget(
    ...
    for (std::size_t targetIndex = 0; targetIndex < state.targets.size(); ++targetIndex) {
        BestTargetResult candidate{};
        if (!evaluateTarget(candidate, state, config, droneMotion, isTurning, isMoving, isDecelerating, isAccelerating, targetIndex, dronePosition, ballistics, acceleration, returningFromManuver)) {
            DEBUG("Invalid calculation for target " << targetIndex);
            continue;
        }

        if (static_cast<int>(targetIndex) == droneMotion.currentTargetIndex) {
    ...
    if (result.targetIndex != droneMotion.currentTargetIndex && droneMotion.currentTargetIndex != -1) {
        if (currentTargetTotalTime < VERY_LARGE_TIME && currentTargetTotalTime <= minTotalTime) {
            if (!evaluateTarget(
                    ...
                    isDecelerating,
                    isAccelerating,
                    static_cast<std::size_t>(droneMotion.currentTargetIndex),
                    dronePosition,
                    ballistics,
                    acceleration,
