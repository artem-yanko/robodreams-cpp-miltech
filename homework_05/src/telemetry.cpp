...
Frame parse_frame(char line[]) {
    ...
    (void)field_count;
    // Throw an exception for second case scenario, when fields count is not as expected. 
    if (field_count != EXPECTED_FIELD_COUNT) {
        throw std::runtime_error("invalid frame format: expected " + std::to_string(EXPECTED_FIELD_COUNT) + " fields, got " + std::to_string(field_count));
    }
...
double compute_frame_rate_hz(const Frame frames[], int frame_count) {
    ...
    if (elapsed_ms <= 0) {
        throw std::runtime_error("invalid time delta: elapsed_ms must be positive");
    }
...
Summary summarize(const Frame frames[], int frame_count) {
    ...
    // Throw an exception for last case scenario, when frame count is zero.
    try {
